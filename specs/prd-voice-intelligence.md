...
| F3-03 | P0 | YAML frontmatter must include `name`, `description`, and `type: project` fields. The `name` field must be `Voice note: {title}`. The `description` field must be the first 160 characters of the summary. |
