...
const SCHEMA_PATH = resolveSchemaPath('data-schema.sql');
let _schemaSQL = null;
...
/**
 * Build WHERE clause parts from a filters object.
 * Handles date ranges (two ISO dates comma-separated → BETWEEN) and
 * regular values (comma-separated → IN).
 * @param {Record<string, string>} filters - column → value map
 * @param {string[]} validColumns - allowed column names
 * @returns {{ whereParts: string[], whereParams: any[] }}
 */
function buildFilterWhere(filters, validColumns) {
  const whereParts = [];
  const whereParams = [];
  for (const [column, value] of Object.entries(filters)) {
    if (column !== 'rowid' && !validColumns.includes(column)) continue;
    const vals = String(value).split(',').map(v => v.trim()).filter(Boolean);
    if (vals.length === 0) continue;

    // Detect date range: exactly 2 values that are both ISO dates
    if (vals.length === 2 && ISO_DATE_RE.test(vals[0]) && ISO_DATE_RE.test(vals[1])) {
      whereParts.push(`"${column}" BETWEEN ? AND ?`);
      whereParams.push(vals[0], vals[1]);
    } else {
...
/**
 * Get a single user table's metadata + counts. Returns null if missing.
 * Shape matches getDataTables entries.
 * @param {string} projectPath
 * @param {string} tableName
 * @returns {{name: string, display_name: string, description: string, row_count: number, column_count: number, created_at: string, updated_at: string} | null}
 */
export function getDataTable(projectPath, tableName) {
  ...
  try {
    ...
    const table = db.prepare('SELECT * FROM _tables WHERE name = ?').get(tableName);
    if (!table) {
      db.close();
      return null;
    }
    ...
    const rowCount = db.prepare(`SELECT COUNT(*) as cnt FROM "${tableName}"`).get();
 ...
 */
export function updateDataTable(projectPath, tableName, opts = {}) {
  const v = validateTableName(tableName);
  if (!v.valid) throw new Error(v.error);
  ...
  try {
    ...
    const result = db.prepare(`UPDATE _tables SET ${fields.join(', ')} WHERE name = ?`).run(...values);
...
export function getTableRows(projectPath, tableName, opts = {}) {
  ...
  const { limit = 100, offset = 0, orderBy, orderDir = 'ASC', filters = {} } = opts;
  ...
  try {
    ...
    // Build WHERE clause from filters (date ranges → BETWEEN, others → IN)
    const { whereParts, whereParams } = buildFilterWhere(filters, colNames);

    const whereClause = whereParts.length > 0 ? ` WHERE ${whereParts.join(' AND ')}` : '';

    const total = db.prepare(`SELECT COUNT(*) as cnt FROM "${tableName}"${whereClause}`).get(...whereParams)?.cnt ?? 0;

    let sql = `SELECT rowid, * FROM "${tableName}"${whereClause}`;
    if (orderBy) {
    ...
    const rows = db.prepare(sql).all(...whereParams, limit, offset);
    db.close();
    return { rows, total };
  } catch (error) {
    db.close();
...
/**
 * Fetch a single row by rowid. Returns null if missing.
 * @param {string} projectPath
 * @param {string} tableName
 * @param {number} rowid
 * @returns {Record<string, any> | null}
 */
export function getRow(projectPath, tableName, rowid) {
  const v = validateTableName(tableName);
  if (!v.valid) throw new Error(v.error);
...
/**
 * Update a single column on many rows in one transaction.
 * @param {string} projectPath
 * @param {string} tableName
 * @param {Array<{rowid: number, value: any}>} updates
 * @param {string} column  - column to update
 */
export function batchUpdateRows(projectPath, tableName, column, updates) {
  const v = validateTableName(tableName);
  if (!v.valid) throw new Error(v.error);
...
/**
 * Convert a JSON filter rule to a SQL WHERE clause fragment.
 * @param {{ column: string, operator: string, value: string }} filter
...
function filterToSql(filter) {
  ...
  switch (filter.operator) {
    ...
    case 'not_equals':
      return { sql: `${col} != ?`, params: [val] };
    case 'contains':
      return { sql: `${col} LIKE ?`, params: [`%${val}%`] };
    case 'not_contains':
      return { sql: `${col} NOT LIKE ?`, params: [`%${val}%`] };
    case 'starts_with':
      return { sql: `${col} LIKE ?`, params: [`${val}%`] };
    case 'ends_with':
      return { sql: `${col} LIKE ?`, params: [`%${val}`] };
...
function buildWhereClause(filters, conjunction = 'AND') {
  ...
  for (const filter of filters) {
    ...
    const { sql, params } = filterToSql(filter);
    fragments.push(sql);
  ...
  const conj = conjunction === 'OR' ? 'OR' : 'AND';
  return {
    whereClause: `WHERE ${fragments.join(` ${conj} `)}`,
    params: allParams,
  };
}

/**
 * Generate a short random ID for views.
 * @returns {string}
 */
 ...
 */
export function getViews(projectPath, tableName) {
  const dbPath = join(projectPath, '.jat', 'data.db');
  if (!existsSync(dbPath)) return [];
  ...
  try {
    ...
    const views = db.prepare(
      'SELECT * FROM _views WHERE table_name = ? ORDER BY name'
    ).all(tableName);

    // Parse JSON filters for each view
    const result = views.map(v => ({
      ...v,
      filters: v.filters ? JSON.parse(v.filters) : [],
      visible_columns: v.visible_columns ? JSON.parse(v.visible_columns) : null,
    }));
 ...
 * @returns {Array<object>}
 */
export function getAllViews(projectPath) {
  const dbPath = join(projectPath, '.jat', 'data.db');
  if (!existsSync(dbPath)) return [];
  ...
  try {
    ...
    const views = db.prepare('SELECT * FROM _views ORDER BY table_name, name').all();
    const result = views.map(v => ({
      ...v,
      filters: v.filters ? JSON.parse(v.filters) : [],
      visible_columns: v.visible_columns ? JSON.parse(v.visible_columns) : null,
 ...
 * @returns {object|null}
 */
export function getView(projectPath, viewId) {
  const dbPath = join(projectPath, '.jat', 'data.db');
  if (!existsSync(dbPath)) return null;
  ...
  try {
    ...
    return {
      ...
      visible_columns: view.visible_columns ? JSON.parse(view.visible_columns) : null,
    };
  } catch (error) {
    db.close();
    throw error;
 ...
 * @returns {object}
 */
export function createView(projectPath, tableName, data) {
  const v = validateTableName(tableName);
  if (!v.valid) throw new Error(v.error);
  ...
  try {
    // Ensure _views table exists
    db.exec(`CREATE TABLE IF NOT EXISTS _views (
      id TEXT PRIMARY KEY,
    ...
    db.prepare(`INSERT INTO _views
      (id, table_name, name, display_name, description, filters, filter_conjunction, visible_columns, sort_column, sort_direction, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      tableName,
      data.name || 'Untitled View',
      data.display_name || data.name || 'Untitled View',
      data.description || '',
      JSON.stringify(data.filters || []),
      data.filter_conjunction || 'AND',
      data.visible_columns ? JSON.stringify(data.visible_columns) : null,
    ...
    touchLastTouched(projectPath);
    const created = db.prepare('SELECT * FROM _views WHERE id = ?').get(id);
    db.close();

    return {
      ...created,
      filters: created.filters ? JSON.parse(created.filters) : [],
      visible_columns: created.visible_columns ? JSON.parse(created.visible_columns) : null,
    };
  } catch (error) {
    db.close();
    throw error;
 ...
 * @returns {object}
 */
export function updateView(projectPath, viewId, data) {
  const dbPath = join(projectPath, '.jat', 'data.db');
  if (!existsSync(dbPath)) throw new Error('No data.db found');

  const db = openDb(dbPath);
  try {
    const existing = db.prepare('SELECT * FROM _views WHERE id = ?').get(viewId);
    if (!existing) throw new Error(`View not found: ${viewId}`);
    ...
    for (const [key, value] of Object.entries(data)) {
      if (['id', 'table_name', 'created_at'].includes(key)) continue;
      if (key === 'filters' || key === 'visible_columns') {
        sets.push(`${key} = ?`);
    ...
    db.prepare(`UPDATE _views SET ${sets.join(', ')} WHERE id = ?`).run(...params);
    touchLastTouched(projectPath);

    const updated = db.prepare('SELECT * FROM _views WHERE id = ?').get(viewId);
    db.close();

    return {
      ...updated,
      filters: updated.filters ? JSON.parse(updated.filters) : [],
      visible_columns: updated.visible_columns ? JSON.parse(updated.visible_columns) : null,
    };
  } catch (error) {
    db.close();
    throw error;
 ...
 */
export function deleteView(projectPath, viewId) {
  const dbPath = join(projectPath, '.jat', 'data.db');
  if (!existsSync(dbPath)) throw new Error('No data.db found');

  const db = openDb(dbPath);
  try {
    const result = db.prepare('DELETE FROM _views WHERE id = ?').run(viewId);
    touchLastTouched(projectPath);
    db.close();
...
/**
 * Get rows for a view, applying its filters.
 * @param {string} projectPath
 * @param {string} viewId
 * @param {object} opts - { limit, offset, orderBy, orderDir }
 * @returns {{ rows: any[], total: number, view: object }}
 */
export function getViewRows(projectPath, viewId, opts = {}) {
  const dbPath = join(projectPath, '.jat', 'data.db');
  if (!existsSync(dbPath)) throw new Error('No data.db found');
  ...
  try {
    ...
    const { whereClause, params: whereParams } = buildWhereClause(filters, conjunction);

    const { limit = 100, offset = 0 } = opts;

    // Use view's sort or override from opts
    const orderBy = opts.orderBy || view.sort_column;
    const orderDir = opts.orderDir || view.sort_direction || 'ASC';

    // Count total matching rows
    const countSql = `SELECT COUNT(*) as cnt FROM "${tableName}" ${whereClause}`;
    const total = db.prepare(countSql).get(...whereParams)?.cnt ?? 0;
    ...
    return { rows, total, view: parsedView };
  } catch (error) {
    db.close();
    throw error;
  }
...
// ---------------------------------------------------------------------------
// Resolution (relation columns → display values)
// ---------------------------------------------------------------------------

/**
 * Resolve relation columns in rows by replacing raw rowid values with display text.
 * For each column with semanticType 'relation', looks up the target table's displayColumn.
 * Returns a new array of resolved rows (does not mutate input).
 *
 * @param {string} projectPath
 * @param {string} tableName
 * @param {Array<Record<string, any>>} rows
 * @param {Array<{column_name: string, semantic_type: string, config: any}>} metaRows - from getColumnMetadata()
 * @returns {Array<Record<string, any>>}
 */
export function resolveRelationColumns(projectPath, tableName, rows, metaRows) {
  if (!rows || rows.length === 0 || !metaRows || metaRows.length === 0) return rows;

  // Find relation columns
  const relationCols = metaRows.filter(m => m.semantic_type === 'relation');
  if (relationCols.length === 0) return rows;
  ...
  const db = openDb(dbPath, true);
  try {
    // Build lookup maps: { columnName: { rowid: displayValue } }
    const lookups = {};

    for (const col of relationCols) {
      const config = typeof col.config === 'string' ? (() => { try { return JSON.parse(col.config); } catch { return {}; } })() : (col.config || {});
      const targetTable = config.targetTable;
      const displayColumn = config.displayColumn;
      if (!targetTable || !displayColumn) continue;

      // Verify target table exists
      const exists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name = ?").get(targetTable);
      if (!exists) continue;

      // Fetch all rows from target table
      const targetRows = db.prepare(`SELECT rowid, "${displayColumn}" FROM "${targetTable}"`).all();
      const map = {};
      for (const r of targetRows) {
        map[String(r.rowid)] = r[displayColumn] ?? `Row ${r.rowid}`;
      }
      lookups[col.column_name] = map;
    }

    db.close();

    if (Object.keys(lookups).length === 0) return rows;

    // Map rows, replacing relation column values
    return rows.map(row => {
      const resolved = { ...row };
      for (const [colName, map] of Object.entries(lookups)) {
        const rawVal = resolved[colName];
        if (rawVal != null && rawVal !== '') {
          resolved[colName] = map[String(rawVal)] ?? rawVal;
        }
      }
      return resolved;
    });
    ...
    try { db.close(); } catch {}
    return rows; // fallback to unresolved
  }
}

// ---------------------------------------------------------------------------
// System Tables (read-only access to tasks.db)
// ---------------------------------------------------------------------------

const SYSTEM_TABLES = ['tasks', 'dependencies', 'labels', 'comments'];

/**
 * Check if a table name is a system table (from tasks.db).
 * @param {string} name
 * @returns {boolean}
 */
export function isSystemTable(name) {
  return SYSTEM_TABLES.includes(name);
}

/**
 * List system tables from tasks.db with row counts and column info.
 * @param {string} projectPath
 * @returns {Array<{name: string, display_name: string, description: string, row_count: number, column_count: number, created_at: string, _system: boolean}>}
 */
export function getSystemTables(projectPath) {
  ...
  try {
    const result = [];
    const descriptions = {
      tasks: 'Task tracking — all project tasks with status, priority, and assignments',
      dependencies: 'Task dependency graph — parent/child blocking relationships',
      labels: 'Task labels — tags attached to tasks',
      comments: 'Task comments — discussion threads on tasks',
    };
    const displayNames = {
      tasks: 'Tasks',
    ...
    for (const name of SYSTEM_TABLES) {
      const exists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name = ?").get(name);
      if (!exists) continue;

      const rowCount = db.prepare(`SELECT COUNT(*) as cnt FROM "${name}"`).get();
...
/**
 * Get schema for a system table from tasks.db.
 * @param {string} projectPath
 * @param {string} tableName
 * @returns {Array<{cid: number, name: string, type: string, notnull: number, dflt_value: any, pk: number}>}
 */
export function getSystemTableSchema(projectPath, tableName) {
  if (!isSystemTable(tableName)) throw new Error(`Not a system table: ${tableName}`);
  const dbPath = join(projectPath, '.jat', 'tasks.db');
  const db = openDb(dbPath, true);
  try {
    const cols = db.pragma(`table_info("${tableName}")`);
...
/**
 * Get rows from a system table (read-only).
 * @param {string} projectPath
 * @param {string} tableName
 * @param {{limit?: number, offset?: number, orderBy?: string, orderDir?: string}} options
 * @returns {{rows: any[], total: number}}
 */
export function getSystemTableRows(projectPath, tableName, options = {}) {
  if (!isSystemTable(tableName)) throw new Error(`Not a system table: ${tableName}`);
  const dbPath = join(projectPath, '.jat', 'tasks.db');
  const db = openDb(dbPath, true);
  try {
    const { limit = 100, offset = 0, orderBy, orderDir = 'ASC', filters = {} } = options;
