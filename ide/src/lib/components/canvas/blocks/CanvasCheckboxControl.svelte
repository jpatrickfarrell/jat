<script lang="ts">
	/**
	 * CanvasCheckboxControl - Boolean toggle for canvas controls
	 *
	 * Renders as: [control name] [checkbox with label]
	 * Settings popover: name, label text
	 */
	import type { ControlBlock, CheckboxControlConfig } from '$lib/types/canvas';
	...
	const config = $derived((block.config || {}) as CheckboxControlConfig);
	const isChecked = $derived(block.value === true || block.value === 'true' || block.value === 1);
	...
	function saveSettings() {
		...
		settingsError = '';
		const updatedConfig: CheckboxControlConfig = {
			label: settingsLabel || undefined,
		};
...
<div class="canvas-checkbox-control">
	...
	{#if settingsOpen}
		...
		>
			<div class="settings-header">
				<span class="settings-title">Checkbox Settings</span>
			...
			<div class="settings-body">
				...
				<label class="settings-field">
					...
					<input type="text" class="field-input" placeholder="Display label next to checkbox" bind:value={settingsLabel} />
				</label>
			</div>
...
<style>
	...
	.canvas-checkbox-control:hover .settings-btn {
		opacity: 1;
	}
