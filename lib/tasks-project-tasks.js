...
export class ProjectTasksBackend extends TaskBackend {
  ...
  async close(taskId, reason = '') {
    ...
    const client = await this.pool.connect();
    try {
      // Check for requester — if set, submit to requester instead of closing.
      const { rows: taskRows } = await client.query(
        'SELECT requester, previous_assignee FROM project_tasks WHERE jat_id = $1', [taskId]
      );
      const requester = taskRows[0]?.requester || null;
      const previousAssignee = taskRows[0]?.previous_assignee || null;

      // If the person who handed the task to the agent IS the requester, skip
      // the submitted state and go straight to accepted.
      const selfAccepted = requester && previousAssignee && previousAssignee === requester;
      ...
      if (requester) {
        ...
        const result = await client.query(
          `UPDATE project_tasks
           SET status = $1, assignee = $2, close_reason = $3, closed_at = NULL, updated_at = $4
           WHERE jat_id = $5
           RETURNING *`,
          [newStatus, requester, reason, now, taskId]
        );
        rows = result.rows;
      } else {
        const result = await client.query(
      ...
      const restored = requester ? rows[0] : await this._restoreAssigneeOnClose(client, taskId, rows[0]);
      return mapRow(restored, this.projectName, this.projectPath);
    } finally {
      client.release();
    }
   ...
   * @param {string} taskId
   * @param {{ author: string, text: string, author_type?: string, comment_type?: string, session_id?: string|null, metadata?: object|null }} comment
   */
  async addComment(taskId, { author, text, author_type = null, comment_type = null, session_id = null, metadata = null }) {
    const client = await this.pool.connect();
    try {
      const { rows: taskRows } = await client.query(
      ...
      const { rows } = await client.query(
        ...
        [uuid, author, text, author_type, comment_type, session_id, metadataJson, new Date().toISOString()]
      );
      const r = rows[0];
      return {
        id: String(r.id),
