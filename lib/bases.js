...
function resolveBaseRef(projectPath, baseName, visitedBaseIds, collapsible = false) {
  try {
    ...
    let refContent = renderBaseContent(projectPath, base);
 ...
 * Uses serializeCanvasToMarkdown for block-based content, with @-reference resolution.
 */
export function renderBase(projectPath, id, opts = {}) {
  const base = getBase(projectPath, id);
  if (!base) throw new Error(`Base "${id}" not found`);

  let content = renderBaseContent(projectPath, base);
  ...
  return {
    id: base.id,
    name: base.name,
    source_type: inferSourceType(base),
    content,
 ...
 * This is the unified content renderer for all base types.
 */
function renderBaseContent(projectPath, base) {
  const blocks = base.blocks || [];
  const sourceType = inferSourceType(base);

  // For data_table bases with no table_view block, use context_query from source_config
  if (sourceType === 'data_table') {
    const hasTableView = blocks.some(b => b.type === 'table_view');
    if (!hasTableView) {
      const contextQuery = base.source_config?.context_query;
      if (contextQuery) {
        return renderDataTableQuery(projectPath, contextQuery);
      }
    }
  }

  // Serialize blocks to markdown (reusing canvas serialization logic)
