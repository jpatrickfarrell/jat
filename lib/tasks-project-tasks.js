...
export class ProjectTasksBackend extends TaskBackend {
  ...
  /**
   * Statuses that count as "closed" in project_tasks (terminal states).
   * @type {Set<string>}
   */
  static TERMINAL_STATUSES = new Set(['completed', 'accepted', 'rejected', 'wontfix', 'closed']);
  ...
  async update(taskId, updates) {
    ...
    try {
      ...
      // If this update transitioned status to a terminal state (completed/accepted/
      // rejected/wontfix/closed), run the same assignee-restore the close() path
      // uses. Covers IDE paths that close tasks via update({status}) rather than
      // close() — e.g. kanban drag, bulk actions.
      if (updates.status && ProjectTasksBackend.TERMINAL_STATUSES.has(rows[0].status)) {
