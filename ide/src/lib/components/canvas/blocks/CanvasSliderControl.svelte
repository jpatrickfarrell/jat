<script lang="ts">
	/**
	 * CanvasSliderControl - Range slider for canvas controls
	 *
	 * Renders as: [control name] [===|=====] [42]
	 * Settings popover: name, min, max, step
	 */
	import type { ControlBlock, SliderControlConfig } from '$lib/types/canvas';
	...
	const config = $derived((block.config || { min: 0, max: 100, step: 1 }) as SliderControlConfig);
	const currentValue = $derived(block.value != null ? Number(block.value) : config.min);

	// Settings popover state
	let settingsOpen = $state(false);
	let settingsRef = $state<HTMLDivElement | null>(null);
	let settingsName = $state('');
	let settingsMin = $state(0);
	let settingsMax = $state(100);
	let settingsStep = $state(1);
	...
	function openSettings() {
		...
		settingsMin = config.min ?? 0;
		settingsMax = config.max ?? 100;
		settingsStep = config.step ?? 1;
		settingsError = '';
		settingsOpen = true;
	}
	...
	function saveSettings() {
		...
		settingsError = '';
		const updatedConfig: SliderControlConfig = {
			min: settingsMin,
			max: settingsMax,
			step: settingsStep,
		};
		...
		if (clampedValue < settingsMin) clampedValue = settingsMin;
		if (clampedValue > settingsMax) clampedValue = settingsMax;
		...
		if (trimmedName) {
			onControlChange(trimmedName, clampedValue);
		}
		settingsOpen = false;
	}
...
<div class="canvas-slider-control">
	...
	{#if settingsOpen}
		...
		>
			<div class="settings-header">
				<span class="settings-title">Slider Settings</span>
			...
			<div class="settings-body">
				...
				<label class="settings-field">
					...
					<input type="number" class="field-input" bind:value={settingsMin} />
				</label>
				<label class="settings-field">
					<span class="field-label">Max</span>
					<input type="number" class="field-input" bind:value={settingsMax} />
				</label>
				<label class="settings-field">
					<span class="field-label">Step</span>
					<input type="number" class="field-input" min="0.001" step="any" bind:value={settingsStep} />
				</label>
			</div>
...
<style>
	...
	.slider-input::-webkit-slider-thumb {
		...
		border: 2px solid oklch(0.50 0.12 200);
		cursor: pointer;
	...
	.canvas-slider-control:hover .settings-btn {
		opacity: 1;
	}
	...
	/* Settings Popover — matches CanvasSelectControl */
	.settings-popover {
		position: absolute;
		top: 100%;
		left: 0;
