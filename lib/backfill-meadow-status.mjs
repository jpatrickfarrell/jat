...
// Map each non-open SQLite row to the PT status we want it to end up with.
// Open rows were intentionally imported as status='dev' and should stay that way.
const updates = rows
  .filter((r) => r.status !== 'open')
  .map((r) => ({
    jat_id: r.id,
    pt_status: r.status,  // post-unification: SQLite and PT share identical status vocabulary
