...
/**
 * Render a view's filtered rows as a markdown table (for agent context injection).
 * Uses the view's context_query if set, otherwise applies view filters.
 * @param {string} projectPath
 * @param {string} viewId
 * @param {string|null} contextQueryOverride
 * @returns {{ view_name: string, table_name: string, content: string, token_estimate: number }}
 */
export function renderDataView(projectPath, viewId, contextQueryOverride = null) {
  ...
  try {
    ...
    // Determine query: override > view context_query > build from filters
    let sql = contextQueryOverride || view.context_query;

    if (sql) {
      // Use explicit SQL query
      const trimmed = sql.trim();
      if (!/^SELECT\b/i.test(trimmed)) {
        db.close();
    ...
    // Build filtered query from view definition
...
/**
 * Build WHERE clause parts from view filter rules.
 * @param {Array<{column: string, operator: string, value?: string}>} filters
 * @returns {{ whereParts: string[], params: any[] }}
 */
function buildViewWhere(filters) {
  ...
  for (const f of filters) {
    ...
    switch (f.operator) {
      case 'equals':
        whereParts.push(`${col} = ?`); params.push(val); break;
      case 'not_equals':
        whereParts.push(`${col} != ?`); params.push(val); break;
      case 'contains':
        whereParts.push(`${col} LIKE ?`); params.push(`%${val}%`); break;
      case 'not_contains':
        whereParts.push(`${col} NOT LIKE ?`); params.push(`%${val}%`); break;
      case 'starts_with':
        whereParts.push(`${col} LIKE ?`); params.push(`${val}%`); break;
      case 'ends_with':
        whereParts.push(`${col} LIKE ?`); params.push(`%${val}`); break;
      ...
      case 'less_equal':
        whereParts.push(`${col} <= ?`); params.push(val); break;
      case 'is_empty':
        whereParts.push(`(${col} IS NULL OR ${col} = '')`); break;
      case 'is_not_empty':
        whereParts.push(`(${col} IS NOT NULL AND ${col} != '')`); break;
      default:
        whereParts.push(`${col} = ?`); params.push(val); break;
    }
  }
