					...
					{#each Array.from(nestedGroupedTasks.entries()) as [projectKey, epicMap]}
						...
						{#if !isProjectCollapsed}
							{#each Array.from(epicMap.entries()) as [epicKey, epicTasks]}
								...
								{@const allChildrenClosed = closedChildrenCount === totalChildrenCount && totalChildrenCount > 0}
								{@const isEpicActuallyComplete = allChildrenClosed && parentTask?.status === 'closed'}
								{@const isEpicReadyForVerification = allChildrenClosed && parentTask?.status !== 'closed'}
								{@const hasReadyChildren = epicTasks.some(t => t.status === 'open' && !t.depends_on?.some(d => d.status !== 'closed'))}
								{@const canRunEpic = !isRunningEpic && !isEpicActuallyComplete && (hasReadyChildren || isEpicReadyForVerification)}
											...
											>
												<div class="flex items-center gap-0 pl-4">
													...
													{#if totalChildrenCount > 1}
														...
														<div class="flex items-center gap-2 ml-3" title="{isEpicReadyForVerification ? `${closedChildrenCount}/${totalChildrenCount} children complete — epic ready for verification` : `${closedChildrenCount} of ${totalChildrenCount} tasks completed`}">
															<!-- Checkmark for fully complete / Rocket for ready-to-verify -->
															{#if isEpicActuallyComplete}
															...
															{:else if isEpicReadyForVerification}
															...
															<div class="w-20 h-1.5 bg-base-content/10 rounded-full overflow-hidden {isRunningEpic ? 'ring-1 ring-primary/30' : ''}">
																<div
																	class="h-full rounded-full transition-all duration-300 {isRunningEpic ? 'animate-pulse' : ''} {isEpicActuallyComplete ? 'bg-success' : isEpicReadyForVerification ? '' : isRunningEpic ? 'bg-warning' : ''}"
																	style="width: {progressPercent}%; {isEpicReadyForVerification ? 'background: oklch(0.65 0.15 200);' : !isEpicActuallyComplete && !isRunningEpic ? `background: ${epicVisual.accent};` : ''}"
																></div>
															</div>
															<!-- X/Y complete text -->
													...
													<!-- Run/Verify Epic button -->
													{#if !isEpicActuallyComplete}
														<button
															class="inline-flex items-center h-6 px-2 text-xs font-medium rounded gap-1 ml-2 bg-transparent transition-colors {isEpicReadyForVerification ? 'text-info hover:bg-info/20 hover:text-info' : 'text-base-content/70 hover:bg-primary/20 hover:text-primary'} {canRunEpic ? '' : 'opacity-40 cursor-not-allowed'}"
															onclick={(e) => {
																e.stopPropagation();
																if (canRunEpic) {
																	if (isEpicReadyForVerification) {
																		handleSpawnSingle(epicKey);
																	} else {
															...
															disabled={!canRunEpic}
															title={isRunningEpic ? 'Epic is already running' : isEpicReadyForVerification ? 'All children complete — spawn agent to verify epic' : !hasReadyChildren ? 'No ready tasks to run' : 'Launch Epic Swarm'}
														>
															{#if isEpicReadyForVerification}
