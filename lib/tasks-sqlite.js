...
export class SqliteTaskBackend extends TaskBackend {
  ...
  addComment(taskId, author, text, projectPath) {
    const found = findProjectForTask(taskId, projectPath, () => this.getProjects());
    if (!found) throw new Error(`Task not found: ${taskId}`);
    const { db, project } = found;
    ...
    try {
      ...
      db.prepare('INSERT INTO comments (id, issue_id, author, text, created_at) VALUES (?, ?, ?, ?, ?)')
        .run(id, taskId, author, text, ts);

      touchLastTouched(project.path);
      const comment = { id, author, text, created_at: ts };
      db.close();
      return comment;
    } catch (error) {
      db.close();
