 ...
 *   - status values mapped (open→dev, closed→completed)
...
async function importToProjectTasks({ pool, data, onProgress, defaultStatus }) {
  ...
  try {
    ...
    // Phase 1: Map all task rows (generates UUIDs, maps columns)
    emit('importing_tasks', `Mapping ${data.tasks.length} tasks...`, 40, {
      current: 0,
      total: data.tasks.length,
    });
    const mappedRows = data.tasks.map((t) => mapTaskRow(t, { defaultStatus }));
    ...
    // Phase 2: set parent_id using the jat_id → UUID map
    const parents = data.tasks.filter((t) => t.parent_id);
    if (parents.length > 0) {
      emit('importing_parents', `Linking ${parents.length} parent relationships...`, 62);
      for (const t of parents) {
        const parentUuid = idMap.get(t.parent_id);
    ...
    // Phase 3: comments → project_tasks_comments (UUID-based)
    emit('importing_comments', `Importing ${data.comments.length} comments...`, 74);
    for (const c of data.comments) {
    ...
    // Phase 4: labels → project_tasks.labels TEXT[] column
    // Group labels by task, then UPDATE the labels array
    emit('importing_labels', `Importing ${data.labels.length} labels...`, 80);
 ...
 * @property {string}  [importStatus]    - Override status for all imported rows (e.g. 'dev', 'submitted')
 * @property {string}  [targetTable]     - Postgres table to import into:
 *                                         'tasks' = JAT-owned table (legacy default, full schema),
 *                                         'project_tasks' = existing JST table (ALTER TABLE + column mapping).
 *                                         Default: 'tasks'. IDE wizard passes 'project_tasks'.
...
export async function graduateProject(opts) {
  ...
  if (useProjectTasks) {
    ...
    try {
      ...
      // 5. Empty-target safety check (counts source='jat' rows only)
      ...
      if (existingCount > 0 && !force) {
        throw new Error(
          `project_tasks already contains ${existingCount} JAT-sourced row(s). ` +
          `Use --force to bypass if you are sure the existing rows won't conflict.`
        );
      }

      // 6. Atomic import with column mapping
      await importToProjectTasks({
        pool,
        data,
        onProgress,
