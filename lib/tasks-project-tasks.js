...
export class ProjectTasksBackend extends TaskBackend {
    ...
    try {
      ...
      try {
        const { rows: cRows } = await client.query(
          `SELECT id, author, author_type, comment_type, session_id, text, created_at
   ...
   * @param {string} taskId
   * @param {{ author: string, text: string, author_type?: string, comment_type?: string, session_id?: string|null }} comment
   */
  async addComment(taskId, { author, text, author_type = null, comment_type = null, session_id = null }) {
    const client = await this.pool.connect();
    try {
      ...
      const { rows } = await client.query(
        `INSERT INTO project_tasks_comments (task_id, author, text, author_type, comment_type, session_id, created_at)
         VALUES ($1::uuid, $2, $3, $4, $5, $6, $7)
         RETURNING id, author, text, author_type, comment_type, session_id, created_at`,
        [uuid, author, text, author_type, comment_type, session_id, new Date().toISOString()]
      );
      const r = rows[0];
      return {
        id: String(r.id),
