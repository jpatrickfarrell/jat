<script lang="ts">
	...
	function openEditDrawer(program: AgentProgramWithStatus) {
		...
		editForm = {
			...
			completionModel: program.completionModel,
			completionCommand: program.completionCommand
		};
		editError = null;
	}
				...
				<div class="form-section">
					...
					<div class="form-group">
						...
						<select
							...
							class="select select-bordered w-full"
							bind:value={editForm.completionModel}
						>
							<option value="">Same as start (no change)</option>
							{#each editForm.models ?? [] as model}
								<option value={model.shortName}>{model.name} ({model.shortName})</option>
							{/each}
						</select>
						<p class="form-hint">When set, the session resumes with this model before running completion</p>
					</div>
