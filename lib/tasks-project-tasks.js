...
function mapRow(row, projectName, projectPath) {
  ...
  return {
    ...
    // updated_by — last editor identity (jat-t5pvg). Shape: { email, name }.
    // Only present on schemas with the 20260501 migration; null otherwise.
    updated_by:     row.updated_by || null,
    updated_by_id:  row.updated_by_id || null,
    comment_count:  typeof row.comment_count === 'number' ? row.comment_count : (parseInt(row.comment_count, 10) || 0),
    milestone_id:   row.milestone_id || null,
    milestone_name: row.milestone_name || null,
    // Feedback context fields — present on JST/jat-feedback schema rows
...
export class ProjectTasksBackend extends TaskBackend {
  ...
  constructor(options = {}) {
    ...
    // Resolved lazily: true if project_tasks has updated_by JSONB column
    // (added by jat-t5pvg migration 20260501000000_project_tasks_updated_by).
    this._hasUpdatedByColumn = null;
    // Resolved lazily: true if this project has milestone_tasks + milestones tables
    this._hasMilestoneTables = null;
  }
  ...
  async _detectUpdatedByColumn() {
    if (this._hasUpdatedByColumn === null) {
      const client = await this.pool.connect();
      try {
        const { rows } = await client.query(
          `SELECT 1 FROM information_schema.columns
        ...
        this._hasUpdatedByColumn = rows.length > 0;
      } finally {
        client.release();
      }
    }
    return this._hasUpdatedByColumn;
  }

  async _detectMilestoneTables() {
    if (this._hasMilestoneTables === null) {
      const client = await this.pool.connect();
      try {
        const { rows } = await client.query(
          `SELECT 1 FROM information_schema.tables
           WHERE table_name='milestone_tasks' AND table_schema='public'`
        );
        this._hasMilestoneTables = rows.length > 0;
      } finally {
        client.release();
      }
    }
    return this._hasMilestoneTables;
  }
  ...
  async list(options = {}) {
    ...
    const hasMilestones = await this._detectMilestoneTables();
    const milestoneJoin = hasMilestones
      ? `LEFT JOIN (
           SELECT DISTINCT ON (mt.task_id)
             mt.task_id,
             m.id   AS milestone_id,
             m.name AS milestone_name
           FROM milestone_tasks mt
           JOIN milestones m ON m.id = mt.milestone_id
           ORDER BY mt.task_id, m.sort_order ASC NULLS LAST
         ) ms ON ms.task_id = pt.id`
      : '';
    const milestoneSelect = hasMilestones
      ? 'ms.milestone_id, ms.milestone_name'
      : 'NULL::uuid AS milestone_id, NULL::text AS milestone_name';
    const sql = `
      SELECT pt.*,
      ...
      LEFT JOIN (
        ...
        WHERE COALESCE(external, true) = true
        GROUP BY task_id
      ) cc ON cc.task_id = pt.id
      ${milestoneJoin}
      ${where}
      ORDER BY
        CASE pt.priority
      ...
      if (hasIdentity) {
        ...
        await client.query(`
          ...
          [
            ...
            assignee, command, agent_program, model, schedule_cron, next_run_at,
            due_date, labelsArr, labelsText, now, now,
            legacyRequester, parentUuid, internal,
          ]
        );
      }
  ...
  async update(taskId, updates) {
    const [hasIdentity, hasUpdatedBy] = await Promise.all([
      this._detectIdentityColumns(),
      this._detectUpdatedByColumn(),
    ]);
    const ALLOWED_BASE = [
      ...
      'close_reason', 'closed_at', 'commit_hash', 'internal',
    ];
    ...
    const identityExtra = hasIdentity
      ? ['requester', 'requester_id', 'approver', 'approver_id']
      : ['requester'];  // legacy: TEXT only
    const updatedByExtra = hasUpdatedBy ? ['updated_by'] : [];
    const ALLOWED = new Set([...ALLOWED_BASE, ...identityExtra, ...updatedByExtra]);
    // JSONB columns — value is a JS object (normalized) or null, cast to ::jsonb.
    const jsonbFields = hasIdentity ? ['requester', 'approver'] : [];
    if (hasUpdatedBy) jsonbFields.push('updated_by');
    const JSONB_FIELDS = new Set(jsonbFields);
    ...
    try {
      ...
      const selectSql = hasIdentity
        ? `SELECT approver, approver_id, requester, requester_id, creator, creator_id,
                  previous_assignee, previous_assignee_id, internal
             FROM project_tasks WHERE jat_id = $1`
        : `SELECT requester, previous_assignee, previous_assignee_id, internal
             FROM project_tasks WHERE jat_id = $1`;
      const { rows: taskRows } = await client.query(selectSql, [taskId]);
      const tr = taskRows[0] || {};

      // Internal tasks bypass stakeholder routing — close immediately.
      if (tr.internal) {
        const result = await client.query(
          `UPDATE project_tasks
             SET status = 'closed', closed_at = $1, close_reason = $2, updated_at = $1
           WHERE jat_id = $3 RETURNING *`,
          [now, reason, taskId]
        );
        if (!result.rows.length) throw new Error(`Task not found: ${taskId}`);
        const restored = await this._restoreAssigneeOnClose(client, taskId, result.rows[0]);
        return mapRow(restored, this.projectName, this.projectPath);
      }
   ...
   */
  /**
   * List all milestones for this project.
   * Returns an array of { id, name, sort_order } objects.
   */
  async listMilestones() {
    ...
    try {
      const { rows } = await client.query(
        `SELECT m.id, m.name, m.sort_order, m.status,
   ...
   * Replaces any existing link — one milestone per task in the UI.
   *
   * @param {string} taskId        - JAT text id (e.g. "meadow-abc12") or UUID
   * @param {string|null} milestoneId - milestone UUID, or null to clear
   */
  async setMilestone(taskId, milestoneId) {
    const hasMilestones = await this._detectMilestoneTables();
    if (!hasMilestones) throw new Error('Milestone tables not available for this project');

    const client = await this.pool.connect();
    try {
      // Resolve task UUID from jat_id if needed
      let taskUuid = taskId;
      if (!taskId.match(/^[0-9a-f]{8}-/)) {
        const { rows } = await client.query(
      ...
      // Delete any existing milestone link
      await client.query(`DELETE FROM milestone_tasks WHERE task_id = $1`, [taskUuid]);

      if (milestoneId) {
        await client.query(
          `INSERT INTO milestone_tasks (task_id, milestone_id) VALUES ($1, $2)`,
          [taskUuid, milestoneId]
        );
      }
    } finally {
      client.release();
