...
export class ProjectTasksBackend extends TaskBackend {
  ...
  async update(taskId, updates) {
    ...
    for (const [key, val] of Object.entries(updates)) {
      ...
      params.push(key === 'priority' ? priorityToText(val) :
                  key === 'status'   ? statusToProjectTasks(val, null) :
                  val);
    ...
    try {
      const { rows } = await client.query(
        `UPDATE project_tasks
         SET status = 'completed', closed_at = $1, close_reason = $2, updated_at = $1
         WHERE jat_id = $3
         RETURNING *`,
        [now, reason, taskId]
      );
      ...
      // Restore the prior assignee (e.g. the Meadow user who clicked "take it"
      // before the agent was spawned). The BEFORE UPDATE trigger stashed the
      // original assignee_id into previous_assignee_id on spawn; this returns
      // the task to its original owner on completion.
      if (rows[0].previous_assignee_id) {
        const { rows: restored } = await client.query(
          `UPDATE project_tasks
           SET assignee_id = $1, updated_at = $2
           WHERE jat_id = $3
           RETURNING *`,
          [rows[0].previous_assignee_id, now, taskId]
        );
        if (restored.length) return mapRow(restored[0], this.projectName, this.projectPath);
      }
      return mapRow(rows[0], this.projectName, this.projectPath);
    } finally {
      client.release();
