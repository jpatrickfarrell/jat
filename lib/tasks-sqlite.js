...
function ensureStatusCheckConstraint(db) {
  ...
  try {
    ...
    const newCols = ['id','title','description','notes','status','priority','issue_type','assignee','previous_assignee','requester','reserved_files','parent_id','command','agent_program','model','schedule_cron','next_run_at','due_date','labels_text','internal','created_at','updated_at','closed_at','close_reason','source','source_item_id','metadata'];
...
export class SqliteTaskBackend extends TaskBackend {
  ...
  /** @returns {import('./tasks-backend.js').Task} */
  create(opts) {
    const { projectPath, title, description = '', type = 'task', priority = 2, labels = [], deps = [], assignee = null, notes = '', id: explicitId, command, agent_program, model, schedule_cron, next_run_at, due_date, source, source_item_id, metadata, requester = null } = opts;
    ...
    try {
      db.prepare(`
        INSERT INTO ${table} (id, title, description, notes, status, priority, issue_type, assignee, requester, command, agent_program, model, schedule_cron, next_run_at, due_date, source, source_item_id, metadata, created_at, updated_at)
        VALUES (?, ?, ?, ?, 'open', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, title, description, notes, priority, type, assignee, requester || null, command || null, agent_program || null, model || null, schedule_cron || null, next_run_at || null, due_date || null, source || null, source_item_id || null, metadataStr, ts, ts);
  ...
  update(taskId, updates) {
    ...
    try {
      const allowedFields = ['title', 'description', 'notes', 'status', 'priority', 'issue_type', 'assignee', 'requester', 'command', 'agent_program', 'model', 'schedule_cron', 'next_run_at', 'due_date', 'internal', 'source', 'source_item_id', 'metadata'];
      const sets = [];
      const params = [];
  ...
  close(taskId, reason = '', projectPath) {
    ...
    try {
      const ts = now();
      const row = db.prepare(`SELECT requester, previous_assignee FROM ${table} WHERE id = ?`).get(taskId);

      if (row && row.requester) {
        // If the person who handed the task to the agent IS the requester, they've
        // already accepted the work — go straight to accepted rather than submitted.
        const selfAccepted = row.previous_assignee && row.previous_assignee === row.requester;
        if (selfAccepted) {
          db.prepare(`UPDATE ${table} SET status = 'accepted', assignee = ?, close_reason = ?, closed_at = NULL, updated_at = ? WHERE id = ?`)
            .run(row.requester, reason, ts, taskId);
        } else {
        ...
        // Restore the prior assignee when no requester is set.
