...
function ensureApproverAndCreatorColumns(db) {
  const cols = db.pragma('table_info(tasks)');
  if (cols.length === 0) return;
  const names = cols.map((/** @type {any} */ c) => c.name);

  // Rename legacy `requester` → `approver`, or add `approver` if neither exists.
  if (!names.includes('approver')) {
    if (names.includes('requester')) {
      db.exec('ALTER TABLE tasks RENAME COLUMN requester TO approver');
    } else {
      db.exec('ALTER TABLE tasks ADD COLUMN approver TEXT');
    }
  } else if (names.includes('requester')) {
    // Both present (shouldn't normally happen): copy non-null requester values
    // into approver where approver is empty, then drop the legacy column.
    db.exec('UPDATE tasks SET approver = requester WHERE approver IS NULL AND requester IS NOT NULL');
    db.exec('ALTER TABLE tasks DROP COLUMN requester');
  }

  if (!names.includes('creator')) {
    db.exec('ALTER TABLE tasks ADD COLUMN creator TEXT');
    // Backfill is best-effort: no history log exists, so creator stays NULL on
    // legacy rows. New writes populate creator via the CLI/API.
  }
}
...
function ensureSourceColumns(db) {
  ...
  if (!names.includes('source_item_id')) {
    db.exec('ALTER TABLE tasks ADD COLUMN source_item_id TEXT');
  }
...
function ensureStatusCheckConstraint(db) {
  ...
  try {
    ...
    const newCols = ['id','title','description','notes','status','priority','issue_type','assignee','previous_assignee','approver','creator','reserved_files','parent_id','command','agent_program','model','schedule_cron','next_run_at','due_date','labels_text','internal','created_at','updated_at','closed_at','close_reason','source','source_item_id','metadata'];
...
function ensureCommentsSchema(db) {
  ...
  const hasExternal = colNames.includes('external');

  if (idIsText && hasAllNewCols && hasExternal) return;

  // Fast path: structure is fine but the `external` column was added later.
  // ALTER TABLE ADD COLUMN is cheap and in-place in SQLite, no rebuild needed.
  if (idIsText && hasAllNewCols && !hasExternal) {
    db.exec(`ALTER TABLE comments ADD COLUMN external INTEGER NOT NULL DEFAULT 1`);
    return;
  }
...
function ensureFts(db) {
  ...
  if (hasFts) {
    // Rebuild if the index is missing the `id` column (added to fix project-prefix search).
    const ftsCols = db.pragma('table_info(tasks_fts)');
    const hasId = ftsCols.some((/** @type {any} */ c) => c.name === 'id');
    if (hasId) return;
    // Drop stale index + triggers so we can rebuild with the id column.
    db.exec(`
  ...
  db.exec(`
    CREATE VIRTUAL TABLE tasks_fts USING fts5(
      id, title, description, labels_text,
      content=tasks, content_rowid=rowid,
      tokenize='porter unicode61'
    );

    INSERT INTO tasks_fts(rowid, id, title, description, labels_text)
    SELECT rowid, id, title, COALESCE(description, ''), COALESCE(labels_text, '')
    FROM tasks;

    CREATE TRIGGER IF NOT EXISTS tasks_fts_ai AFTER INSERT ON tasks BEGIN
      INSERT INTO tasks_fts(rowid, id, title, description, labels_text)
      VALUES (new.rowid, new.id, new.title, COALESCE(new.description, ''), COALESCE(new.labels_text, ''));
    END;

    CREATE TRIGGER IF NOT EXISTS tasks_fts_bd BEFORE DELETE ON tasks BEGIN
      INSERT INTO tasks_fts(tasks_fts, rowid, id, title, description, labels_text)
      VALUES('delete', old.rowid, old.id, old.title, COALESCE(old.description, ''), COALESCE(old.labels_text, ''));
    END;
...
function getWritableDb(projectPath) {
  ...
  const db = openDb(dbPath);
  ensureApproverAndCreatorColumns(db);
...
function searchLike(db, table, query, options) {
  ...
  let sql = `SELECT *, 0 AS relevance FROM ${table} WHERE (id LIKE ? OR title LIKE ? OR description LIKE ?)`;
...
export class SqliteTaskBackend extends TaskBackend {
  ...
  getProjects() {
    ...
    if (jatConfig?.projects) {
      for (const [name, config] of Object.entries(jatConfig.projects)) {
        ...
        // is sqlite so unmarked projects fall through unchanged.  Record the
        // path so the ~/code scan below doesn't re-discover a stale SQLite
        // file left behind before graduation.
        const backend = /** @type {any} */ (config).backend;
        if (backend && backend !== 'sqlite') {
          if (projectPath) graduatedPaths.add(projectPath);
          continue;
        }
    ...
    if (existsSync(codeDir)) {
      try {
        for (const entry of readdirSync(codeDir, { withFileTypes: true })) {
          ...
          if (graduatedPaths.has(projectPath)) continue;
  ...
  /** @returns {import('./tasks-backend.js').Task} */
  create(opts) {
    const { projectPath, title, description = '', type = 'task', priority = 2, labels = [], deps = [], assignee = null, notes = '', id: explicitId, command, agent_program, model, schedule_cron, next_run_at, due_date, source, source_item_id, metadata, approver = null, creator = null, requester = null } = opts;
    ...
    const metadataStr = metadata == null
      ? null
      : typeof metadata === 'string' ? metadata : JSON.stringify(metadata);

    // Back-compat: callers still passing `requester` map it to `approver`.
    const approverVal = approver ?? requester ?? null;

    // Runtime invariant: creator must be set on every new task.
    // Enable JAT_STRICT_IDENTITY=1 to throw; otherwise warns.
    if (creator === null || creator === undefined) {
      const msg = `[JAT] createTask("${title}") called without creator — route through buildTaskIdentity(). Stack: ${new Error().stack?.split('\n')[2]?.trim() ?? '?'}`;
      if (process.env.JAT_STRICT_IDENTITY) {
        throw new Error(msg);
      }
      console.warn(msg);
    }
    ...
    try {
      db.prepare(`
        INSERT INTO ${table} (id, title, description, notes, status, priority, issue_type, assignee, approver, creator, command, agent_program, model, schedule_cron, next_run_at, due_date, source, source_item_id, metadata, created_at, updated_at)
        VALUES (?, ?, ?, ?, 'open', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, title, description, notes, priority, type, assignee, approverVal, creator || null, command || null, agent_program || null, model || null, schedule_cron || null, next_run_at || null, due_date || null, source || null, source_item_id || null, metadataStr, ts, ts);
  ...
  update(taskId, updates) {
    ...
    try {
      const allowedFields = ['title', 'description', 'notes', 'status', 'priority', 'issue_type', 'assignee', 'approver', 'creator', 'command', 'agent_program', 'model', 'schedule_cron', 'next_run_at', 'due_date', 'internal', 'source', 'source_item_id', 'metadata'];
      const sets = [];
      const params = [];

      // Back-compat: accept `requester` as an alias for `approver`.
      if (updates.requester !== undefined && updates.approver === undefined) {
        updates.approver = updates.requester;
      }
  ...
  close(taskId, reason = '', projectPath) {
    ...
    try {
      const ts = now();
      const row = db.prepare(`SELECT approver, creator, previous_assignee FROM ${table} WHERE id = ?`).get(taskId);

      // Route to approver first; fall back to creator. This lets tasks that
      // never had an explicit approver still flow back to whoever created them.
      const routingTarget = row ? (row.approver || row.creator || null) : null;

      if (routingTarget) {
        // If the person who handed the task to the agent IS the routing target,
        // they've already accepted the work — go straight to accepted.
        const selfAccepted = row.previous_assignee && row.previous_assignee === routingTarget;
        if (selfAccepted) {
          db.prepare(`UPDATE ${table} SET status = 'accepted', assignee = ?, close_reason = ?, closed_at = NULL, updated_at = ? WHERE id = ?`)
            .run(routingTarget, reason, ts, taskId);
        } else {
          db.prepare(`UPDATE ${table} SET status = 'submitted', assignee = ?, close_reason = ?, closed_at = NULL, updated_at = ? WHERE id = ?`)
            .run(routingTarget, reason, ts, taskId);
        }
      } else {
        ...
        // Restore the prior assignee when no routing target is set.
  ...
  addComment(taskId, author, text, projectPath, extra = {}) {
    ...
    try {
      ...
      // external defaults true. Anything that's not an explicit `false` stays external.
      const external = extra.external === false ? 0 : 1;

      db.prepare(
        `INSERT INTO comments (id, issue_id, text, author, author_type, comment_type, session_id, metadata, external, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(id, taskId, text, author, authorType, commentType, sessionId, metadata, external, ts);

      touchLastTouched(project.path);
      const comment = {
        id, text, author, author_type: authorType, comment_type: commentType,
        session_id: sessionId, metadata, external: external === 1, created_at: ts,
      };
      db.close();
      return comment;
    } catch (error) {
   ...
   * @param {{ external?: boolean }} [opts] — pass `{external: true}` to return only external comments
   * @returns {Array<object>}
   */
  listComments(taskId, projectPath, opts = {}) {
    const found = findProjectForTask(taskId, projectPath, () => this.getProjects());
    if (!found) throw new Error(`Task not found: ${taskId}`);
    const { db } = found;
    try {
      ensureCommentsSchema(db);
      const params = [taskId];
      let where = 'WHERE issue_id = ?';
      if (opts.external === true) {
        where += ' AND external = 1';
      }
      const rows = db.prepare(
        `SELECT id, text, author, author_type, comment_type, session_id, metadata, external, created_at
         FROM comments ${where} ORDER BY created_at ASC`
      ).all(...params);
      // Surface external as a real boolean for API consumers.
      const out = rows.map((/** @type {any} */ r) => ({ ...r, external: r.external !== 0 }));
      db.close();
      return out;
    } catch (err) {
      db.close();
  ...
  /**
   * Flip a comment to internal. The one-way rule (internal cannot become
   * external) is enforced at the API layer — this helper only writes whatever
   * value it's given.
   *
   * If `projectPath` is given we look in that project's db directly (matches
   * findProjectForTask). Otherwise we walk every registered project until we
   * find the comment.
   *
   * @returns {object|null}
   */
  setCommentExternal(commentId, external, projectPath) {
    const candidates = projectPath
      ? [{ path: projectPath }]
      : this.getProjects();
    for (const project of candidates) {
      const dbPath = join(project.path, '.jat', 'tasks.db');
      ...
      try {
        ...
        db.prepare('UPDATE comments SET external = ? WHERE id = ?').run(external ? 1 : 0, commentId);
        ...
        db.close();
        touchLastTouched(project.path);
        return { ...updated, external: updated.external !== 0 };
      } catch (err) {
        db.close();
        throw err;
      }
  ...
  /** @returns {object|null} */
  getComment(commentId, projectPath) {
    const candidates = projectPath
      ? [{ path: projectPath }]
      : this.getProjects();
    for (const project of candidates) {
      ...
      try {
        ensureCommentsSchema(db);
        const row = db.prepare(
          `SELECT id, text, author, author_type, comment_type, session_id, metadata, external, issue_id, created_at
           FROM comments WHERE id = ?`
        ).get(commentId);
        db.close();
        if (row) return { ...row, external: row.external !== 0 };
      } catch (err) {
        db.close();
        throw err;
      }
