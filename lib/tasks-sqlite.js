...
function ensureStatusCheckConstraint(db) {
  ...
  try {
    ...
    const newCols = ['id','title','description','notes','status','priority','issue_type','assignee','previous_assignee','reserved_files','parent_id','command','agent_program','model','schedule_cron','next_run_at','due_date','labels_text','internal','created_at','updated_at','closed_at','close_reason','source','source_item_id','metadata'];
...
export class SqliteTaskBackend extends TaskBackend {
  ...
  /** @returns {import('./tasks-backend.js').Task} */
  create(opts) {
    const { projectPath, title, description = '', type = 'task', priority = 2, labels = [], deps = [], assignee = null, notes = '', id: explicitId, command, agent_program, model, schedule_cron, next_run_at, due_date, source, source_item_id, metadata } = opts;
    ...
    try {
      db.prepare(`
        INSERT INTO ${table} (id, title, description, notes, status, priority, issue_type, assignee, command, agent_program, model, schedule_cron, next_run_at, due_date, source, source_item_id, metadata, created_at, updated_at)
        VALUES (?, ?, ?, ?, 'open', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, title, description, notes, priority, type, assignee, command || null, agent_program || null, model || null, schedule_cron || null, next_run_at || null, due_date || null, source || null, source_item_id || null, metadataStr, ts, ts);
  ...
  update(taskId, updates) {
    ...
    try {
      const allowedFields = ['title', 'description', 'notes', 'status', 'priority', 'issue_type', 'assignee', 'command', 'agent_program', 'model', 'schedule_cron', 'next_run_at', 'due_date', 'internal', 'source', 'source_item_id', 'metadata'];
      const sets = [];
      const params = [];
