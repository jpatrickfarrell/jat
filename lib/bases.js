...
export function getBases(projectPath, opts = {}) {
  ...
  try {
    ...
    // Check if sort_order column exists (may not yet be migrated)
    const hasSortOrder = (() => {
      try {
        db.prepare('SELECT sort_order FROM bases LIMIT 0').run();
        return true;
      } catch { return false; }
    })();
    ...
    sql += hasSortOrder ? ' ORDER BY sort_order ASC, name ASC' : ' ORDER BY name ASC';
...
/**
 * Batch update sort_order for a list of bases.
 * @param {string} projectPath
 * @param {Array<{id: string, sort_order: number}>} order
 */
export function reorderBases(projectPath, order) {
  const dbPath = join(projectPath, '.jat', 'data.db');
  if (!existsSync(dbPath)) throw new Error('No data.db found');
  ...
  try {
    ensureBasesTable(db);
    const stmt = db.prepare('UPDATE bases SET sort_order = ?, updated_at = ? WHERE id = ?');
    const ts = new Date().toISOString();
    const tx = db.transaction(() => {
      for (const item of order) {
        stmt.run(item.sort_order, ts, item.id);
      }
    });
    tx();
    touchLastTouched(projectPath);
 ...
 * Serializes a base's blocks to markdown for agent injection.
 */
...
function ensureBasesTable(db) {
  ...
  const columnsToAdd = [
    ...
    { name: 'icon', sql: 'ALTER TABLE bases ADD COLUMN icon TEXT' },
    { name: 'sort_order', sql: 'ALTER TABLE bases ADD COLUMN sort_order INTEGER DEFAULT 0' },
  ];
  for (const col of columnsToAdd) {
    try {
