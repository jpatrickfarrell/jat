...
// Track if Hyprland rules have been generated this server lifetime
let hyprlandRulesGenerated = false;
...
export async function GET({ url }) {
	try {
		...
		// Ensure Hyprland border rules are in sync on first load
		if (jatConfig && !hyprlandRulesGenerated) {
			hyprlandRulesGenerated = true;
			generateHyprlandProjectRules(jatConfig).catch(err =>
				console.error('[Projects] Startup Hyprland rule generation failed:', err)
			);
		}
...
async function writeJatConfig(config) {
	try {
		...
		// Regenerate Hyprland border rules (fire-and-forget)
		generateHyprlandProjectRules(config).catch(err =>
			console.error('[Projects] Failed to regenerate Hyprland rules:', err)
		);
...
/**
 * Convert any color format to Hyprland rgb(RRGGBB) hex format.
 * Supports: rgb(RRGGBB), #RRGGBB, oklch(...) -> approximate hex.
 * Returns null if color can't be converted.
 * @param {string} color
 * @returns {string | null}
 */
function toHyprlandRgb(color) {
	if (!color) return null;

	// Already in Hyprland rgb(RRGGBB) format
	const rgbHexMatch = color.match(/^rgb\(([0-9a-fA-F]{6})\)$/);
	if (rgbHexMatch) return color;
	...
	// rgb(R, G, B) CSS format with commas
	const rgbCssMatch = color.match(/^rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)$/);
	if (rgbCssMatch) {
		const hex = [rgbCssMatch[1], rgbCssMatch[2], rgbCssMatch[3]]
			.map(n => parseInt(n).toString(16).padStart(2, '0'))
			.join('');
		return `rgb(${hex})`;
	}

	// oklch() - skip, can't convert without a color library
	if (color.startsWith('oklch(')) return null;

	return null;
}

const HYPRLAND_RULES_FILE = join(homedir(), '.config', 'hypr', 'projects.conf');

/**
 * Generate ~/.config/hypr/projects.conf from current project colors.
 * Hyprland 0.53+ match:title uses full-string regex, so patterns must be PROJECT:.*
 * to match titles like "STEELBRIDGE: Tasks | JAT IDE" or "STEELBRIDGE: jat-AgentName".
 * After writing, reloads Hyprland config if hyprctl is available.
 * @param {object} config - The full JAT projects.json config
 */
async function generateHyprlandProjectRules(config) {
	if (!config?.projects) return;

	const lines = [
		'# Auto-generated by JAT IDE from ~/.config/jat/projects.json',
		'# Do not edit manually — changes will be overwritten when projects are updated.',
		'# Hyprland 0.53+ match:title requires full-string regex (PROJECT:.* not ^PROJECT)',
		''
	];
	...
	for (const [key, projectConfig] of Object.entries(config.projects)) {
		...
		const displayName = (pc.name || key).toUpperCase();
		const activeColor = toHyprlandRgb(pc.active_color);
		const inactiveColor = toHyprlandRgb(pc.inactive_color);

		if (!activeColor) continue;

		const colors = inactiveColor
			? `${activeColor} ${inactiveColor}`
			: activeColor;

		lines.push(`# ${displayName}`);
		lines.push(`windowrule = border_color ${colors}, match:title ${displayName}:.*`);
		lines.push('');
		projectNames.push(displayName);
	}

	// Thicker borders for all project windows
	if (projectNames.length > 0) {
		lines.push('# Thicker borders for project windows');
		lines.push(`windowrule = border_size 3, match:title (${projectNames.join('|')}):.*`);
		lines.push('');
	}

	// Write the file
	const hyprDir = join(homedir(), '.config', 'hypr');
	if (!existsSync(hyprDir)) return; // No Hyprland config dir, skip

	await writeFile(HYPRLAND_RULES_FILE, lines.join('\n'));

	// Reload Hyprland config (non-blocking)
	try {
		await execAsync('hyprctl reload', { timeout: 3000 });
	} catch {
		// Hyprland not running or not available — ignore
	}
}
