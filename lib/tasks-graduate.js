...
function getProjectTasksDepsMigrationSQL() {
  if (_depsMigrationSQL === null) {
    const __dirname = dirname(fileURLToPath(import.meta.url));
    _depsMigrationSQL = readFileSync(
      join(__dirname, 'migrations', 'add_project_tasks_dependencies.sql'),
      'utf-8',
    );
  }
  return _depsMigrationSQL;
}

/**
 * Apply the JAT schema extensions to an existing project_tasks table:
 *   1. Column additions (jat_id, agent_program, ...) from add_jat_columns_…
 *   2. Dependency table from add_project_tasks_dependencies (jat-ikvqh.19)
 * Both migrations are idempotent — IF NOT EXISTS on every DDL.
 *
...
async function applyProjectTasksSchema(pool) {
  ...
  try {
    ...
    await client.query(getProjectTasksDepsMigrationSQL());
  } finally {
    client.release();
  }
}
...
async function importToProjectTasks({ pool, data, onProgress, defaultStatus }) {
  ...
  try {
    ...
    // Phase 5: dependencies → project_tasks_dependencies (added by
    // jat-ikvqh.19 migration). Resolves TEXT jat_ids → UUIDs via idMap.
    // Skips rows where either side didn't get migrated (shouldn't happen
    // on a clean graduation, but could if one side was manually closed
    // before graduation ran).
    const newDeps = data.dependencies.filter(
      (d) => newJatIds.has(d.issue_id) || newJatIds.has(d.depends_on_id),
    );
    emit('importing_dependencies', `Importing ${newDeps.length} dependencies...`, 82);
    for (const d of newDeps) {
      // Resolve both sides: either already-mapped new UUID or an existing
      // jat_id that we need to look up.
      const resolveUuid = async (jatId) => {
        if (idMap.has(jatId)) return idMap.get(jatId);
        if (existingJatIds.has(jatId)) {
      ...
      const taskUuid = await resolveUuid(d.issue_id);
      const depUuid = await resolveUuid(d.depends_on_id);
      if (!taskUuid || !depUuid) continue;     // orphan dep; skip
      if (taskUuid === depUuid) continue;      // self-dep; skip (constraint would reject)
      ...
      try {
        await client.query(
          ...
          [taskUuid, depUuid, d.type || 'blocks'],
        );
      } catch (depErr) {
        // If the target DB predates the migration, surface a clear error
        // rather than half-committing a partial transaction.
        if (String(depErr.message || '').includes('project_tasks_dependencies')) {
          throw new Error(
            'project_tasks_dependencies table missing. Apply migration: ' +
            'lib/migrations/add_project_tasks_dependencies.sql',
          );
        }
        throw depErr;
      }
