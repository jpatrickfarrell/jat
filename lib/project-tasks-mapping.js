...
// ---------------------------------------------------------------------------
// Status mapping: JAT status → project_tasks status
// ---------------------------------------------------------------------------

/** @type {Record<string, string>} */
const STATUS_JAT_TO_PT = {
  open: 'dev',           // JAT 'open' → 'dev' (internal, not yet submitted)
...
/**
 * Map a JAT status value to its project_tasks equivalent.
 * @param {string} jatStatus
 * @param {string} [defaultStatus] - Override default mapping (e.g., 'dev' for all imported tasks)
 * @returns {string}
 */
export function statusToProjectTasks(jatStatus, defaultStatus) {
  if (defaultStatus) return defaultStatus;
  return STATUS_JAT_TO_PT[jatStatus] ?? jatStatus;
}

/**
 * Map a project_tasks status value to its JAT equivalent.
 * @param {string} ptStatus
 * @returns {string}
 */
export function statusToJat(ptStatus) {
  return STATUS_PT_TO_JAT[ptStatus] ?? ptStatus;
}
...
export function mapTaskRow(jatRow, options = {}) {
  return {
    ...
    issue_type: jatRow.issue_type || 'task',
