 ...
 * @property {number|string} id  - Row ID (TEXT nanoid for new rows, INTEGER for legacy)
 * @property {string} author     - Free-text author name
 * @property {string} text       - Comment body (plain text / markdown)
 * @property {boolean} [external] - True (default) = visible to external parties;
 *                                   false = internal-only. One-way: a comment
 *                                   set to internal cannot be flipped back.
 ...
 * @property {string}            status          - "open" | "in_progress" | "waiting" | "blocked" | "submitted" | "accepted" | "deployed" | "closed" | "dev"
 ...
 * @property {string}   [parent_id]         - Parent task jat_id for hierarchical child (auto-assigns child ID)
...
export async function getBackendForProject(nameOrPath) {
  ...
  if (resolved.kind === 'postgres') {
    // Graduated projects write to project_tasks (jat-dplkm.3).
    // Use ProjectTasksBackend which reads from that table with proper column mapping.
    const { ProjectTasksBackend } = await import('./tasks-project-tasks.js');
    const { getProjectConfig } = await import('./projects-config.js');
    const entry = getProjectConfig(nameOrPath);
    const projectName = entry ? entry.name.toLowerCase() : nameOrPath;
    const projectPath = entry?.config?.path
      ? entry.config.path.replace(/^~/, process.env.HOME || '')
      : '';
    backend = new ProjectTasksBackend({
...
/**
 * Return the TaskBackend for the project that owns the given task ID.
 *
 * Task IDs follow the convention `{project}-{suffix}`, e.g. "meadow-q380k".
 * The project name is everything before the first `-`.
 *
 * @param {string} taskId
 * @returns {Promise<TaskBackend>}
 */
export async function getBackendForTaskId(taskId) {
  const dash = typeof taskId === 'string' ? taskId.indexOf('-') : -1;
  if (dash <= 0) throw new Error(`Cannot derive project from task ID: ${taskId}`);
  return getBackendForProject(taskId.slice(0, dash));
}
