 ...
 *   - project_tasks has an `internal` boolean column; true = dev-only, hidden from client views
...
// ---------------------------------------------------------------------------
// Status: post-unification (see lib/task-statuses.js, jat-ch9y0), SQLite and
// Postgres share the identical status vocabulary — no translation needed.
// ---------------------------------------------------------------------------
...
export function mapTaskRow(jatRow, options = {}) {
  return {
    ...
    notes: jatRow.notes || '',
    status: options.defaultStatus || jatRow.status,
  ...
  return {
    sql: `
      INSERT INTO project_tasks (
        ...
        created_at, updated_at, closed_at, close_reason, internal
      ) VALUES (
    ...
    params(row) {
      return [
        ...
        row.internal ?? true, // $22 internal (JAT-synced tasks default to internal)
      ];
    },
  };
}
