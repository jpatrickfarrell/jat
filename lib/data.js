...
export function getTableRows(projectPath, tableName, opts = {}) {
  ...
  try {
    ...
    // Build WHERE clause from filters (column IN (...) for comma-separated values)
    const whereParts = [];
    const whereParams = [];
    for (const [column, value] of Object.entries(filters)) {
      if (column !== 'rowid' && !colNames.includes(column)) continue;
...
export function getSystemTableRows(projectPath, tableName, options = {}) {
  ...
  try {
    ...
    const whereParams = [];
    for (const [column, value] of Object.entries(filters)) {
      if (!colNames.includes(column)) continue; // skip invalid columns
      const vals = String(value).split(',').map(v => v.trim()).filter(Boolean);
      if (vals.length === 0) continue;
      const placeholders = vals.map(() => '?').join(', ');
      whereParts.push(`"${column}" IN (${placeholders})`);
