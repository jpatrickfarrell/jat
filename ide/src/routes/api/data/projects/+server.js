 ...
 *
 * `hasDataDb` is true when either:
 *   - the project's `.jat/data.db` file exists (sqlite backend), OR
 *   - the project is configured with backend="postgres" (graduated to postgres)
 */
...
import { isPostgresProject } from '$lib/server/jat-data.js';
...
export async function GET() {
	try {
		...
		for (const [key, cfg] of Object.entries(projectEntries)) {
			...
			const isPostgres = isPostgresProject(key);
			const hasLocalDataDb = existsSync(join(jatDir, 'data.db'));
			projects.push({
				name: key,
				hasDataDb: isPostgres || hasLocalDataDb,
