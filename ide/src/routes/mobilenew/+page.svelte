<script lang="ts">
	...
	// Session drawer state (mobilenew: SessionCard in drawer instead of MobileSessionFullscreen)
	...
	async function fetchTasks() {
		try {
			...
			const existingIds: Set<string> = new Set(tasks.map((t: Task) => t.id));
										...
										{#if isExpanded}
											...
											>
												<TasksActive
													...
													onMobileCardClick={(sn) => drawerSessionName = sn}
													mobile={true}
												/>
											</div>
										{/if}
...
<!-- MobileSessionDrawer: SessionCard in a swipe-to-dismiss drawer -->
