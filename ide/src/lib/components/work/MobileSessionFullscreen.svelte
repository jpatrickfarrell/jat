<script lang="ts">
	...
	// Direct event actions — bypass Svelte 5 event delegation which breaks when
	// the element is portaled to document.body (delegated onclick/onkeydown
	// handlers attached at the original mount root never fire on the portaled
	// subtree). Use these on every interactive element inside the portal.
	...
	function directKeydown(node: HTMLElement, handler: (e: KeyboardEvent) => void) {
		...
		return { destroy() { node.removeEventListener('keydown', h); } };
	}
	function directInput(node: HTMLElement, handler: (e: Event) => void) {
		node.addEventListener('input', handler);
		return { destroy() { node.removeEventListener('input', handler); } };
	}
	function directBlur(node: HTMLElement, handler: (e: FocusEvent) => void) {
		const h = handler as EventListener;
		node.addEventListener('blur', h);
		return { destroy() { node.removeEventListener('blur', h); } };
	}
	function directMousedown(node: HTMLElement, handler: (e: MouseEvent) => void) {
		const h = handler as EventListener;
		node.addEventListener('mousedown', h);
		return { destroy() { node.removeEventListener('mousedown', h); } };
	}
	function directMouseenter(node: HTMLElement, handler: (e: MouseEvent) => void) {
		const h = handler as EventListener;
...
<div use:portalToBody>
	...
	<div
		...
		role="presentation"
		use:directClick={handleClose}
		use:directKeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleClose(); } }}
	></div>
	...
	>
		...
		<div class="fullscreen-topbar">
			<button class="back-btn" use:directClick={handleClose} aria-label="Close">
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
					<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
				</svg>
			</button>

			<div class="topbar-info" role="button" tabindex="0" use:directClick={() => taskId && onViewTask(taskId)} use:directKeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); taskId && onViewTask(taskId); } }}>
		...
		<div class="fullscreen-output">
			...
			{#if !autoScroll}
				<button class="scroll-bottom-btn" aria-label="Scroll to bottom" use:directClick={scrollToBottom}>
		...
		<div class="fullscreen-input">
				...
				<div class="attachment-row">
					{#each pendingAttachments as att (att.id)}
						<div class="attachment-chip" title={att.path}>
							{#if att.isImage}
								<img class="attachment-thumb" src="/api/work/image/{encodeURIComponent(att.name)}" alt={att.name} />
							{:else}
			...
			{#if sendInputError}
				<div class="input-error">
					...
					<button class="error-dismiss" use:directClick={() => sendInputError = null}>&times;</button>
				</div>
			{/if}
			<div class="input-row">
				<div class="input-wrapper">
					<textarea
						...
						use:directKeydown={handleInputKeydown}
						use:directInput={handleInputChange}
						use:directBlur={() => { setTimeout(() => { showPathAutocomplete = false; }, 200); }}
						...
						<div class="path-autocomplete">
							{#each pathSearchResults as file, i (file.path)}
								<button
									...
									use:directMouseenter={() => pathAutocompleteIndex = i}
									use:directMousedown={(e) => { e.preventDefault(); selectPathResult(file); }}
								>
