 ...
 */
export function attachBaseToTask(projectPath, taskId, baseId) {
  const dbPath = join(projectPath, '.jat', 'bases.db');
  if (!existsSync(dbPath)) throw new Error('No bases.db found');
  ...
  try {
    ...
    db.prepare('INSERT INTO task_bases (task_id, base_id, attached_at) VALUES (?, ?, ?)')
      .run(taskId, baseId, now());
