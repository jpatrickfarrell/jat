<script lang="ts">
	 ...
	 * Today Page
	 *
	 * Today's task view with project selection via TopBar global selector.
	 * Duplicated from /tasks route for customization.
	...
	const filteredOpenTasks = $derived.by(() => {
		...
		return openTasks.filter((task) => {
			// Always show epics (they don't have due dates themselves)
			if (task.issue_type === "epic") return true;
			// Always show in_progress/blocked tasks regardless of filter
...
<svelte:head>
	<title>Today | JAT IDE</title>
	<meta
		name="description"
		content="Today's task view for AI coding agents."
	/>
