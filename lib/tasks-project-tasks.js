  ...
  async close(taskId, reason = '') {
    ...
    try {
      ...
      const { rows: taskRows } = await client.query(
        'SELECT requester FROM project_tasks WHERE jat_id = $1', [taskId]
      );
      ...
      if (requester) {
        const result = await client.query(
          `UPDATE project_tasks
           SET status = 'submitted', assignee = $1, close_reason = $2, closed_at = NULL, updated_at = $3
           WHERE jat_id = $4
           RETURNING *`,
          [requester, reason, now, taskId]
        );
        rows = result.rows;
      } else {
        const result = await client.query(
