...
async function runTests() {
  ...
  // --- 10. markAllInternal flag ---
  console.log('\nmarkAllInternal');
  // Wipe the existing Postgres data so we can re-graduate with the flag.
  const pool2 = new pg.Pool({ connectionString: PG_URL });
  try {
    await pool2.query('DROP TABLE IF EXISTS comments, dependencies, labels, tasks CASCADE');
  } finally {
    await pool2.end();
  }

  const result2 = await graduateProject({
    projectNameOrPath: PROJECT_KEY,
    postgresUrl: PG_URL,
    markAllInternal: true,
  });
  assertEq(result2.status, 'graduated', 'markAllInternal graduation succeeds');

  const pool3 = new pg.Pool({ connectionString: PG_URL });
  try {
    const { rows } = await pool3.query('SELECT id, internal FROM tasks ORDER BY id');
    assertEq(rows.length, 3, 'markAllInternal: 3 tasks in pg');
    assert(rows.every(r => r.internal === true), 'markAllInternal: all tasks have internal=true');
  } finally {
    await pool3.end();
  }
}
